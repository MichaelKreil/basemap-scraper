...
export async function merge_tiles_to_layers(config) {
	const { level, bbox, tiles_folder, geojson_folder } = config;

	mkdirSync(geojson_folder, { recursive: true });
	...
	for (let i = 0; i < tiles.length; i++) {
		...
		const filename = resolve(tiles_folder, `${tile.z}/${tile.x}/${tile.y}.pbf`);
	...
	function LayerFiles() {
		...
		function get(name) {
			...
			let filename = resolve(geojson_folder, name.toLowerCase().replace(/\s/g, '_') + '.geojsonl.gz');
